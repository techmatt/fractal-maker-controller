...
- **Certification [pre-registered]: PASSED on all four arms** — bars + results → `data/v11/{prereg_v11.json, eval_results_v11.json}`.
...
## Backbone search + quantization (2026-08-15) — both CLOSED
- **backbone_search_v1 verdict: KEEP `mobilenetv4_conv_medium`.** The control is the FASTEST (22.45–22.6 s/1k tiles end-to-end; 1.09 s/1k GPU-only vs 1.37–4.07 for every other arm) AND the SMALLEST (8.44M params vs 9.8–31.3M); zero of 45 arm×partition cells beat it with a CI excluding 0; the control's 3-seed spread is ~half the total spread across six backbones — the backbone axis is not clearly larger than the seed axis. fastvit_sa12 tie-above on the robust rule (+25% params, 2.4× GPU cost, descriptively worse on julia:mandelbrot/mb4/mb5) — NOT adopted. Deploy is DECODE-BOUND (backbone 5–17% of score time) — a faster backbone buys ~nothing end-to-end. Caveats: frozen 40-epoch schedule handicaps high-capacity arms; convnextv2/FCMAE unmeasured (`convnextv2_nano` = the cheap future buy). Record → `data/backbone_search/` + the committed report.
- **★★ COMMITTED CORN SCORE FILES HOLD CONDITIONAL SIGMOIDS, NOT PROBABILITIES.** CORN trains cutpoint k only on rows that cleared k−1, so `σ(logit_k)` is CONDITIONAL; unconditional P(≥k) = the running product. Reading `eval_scores_backbone_v1.jsonl` (or any per-cutpoint score column) as bare P(≥k) understates sparse cutpoints — measured ~7 points of AUC(≥4). **Product-read both sides before comparing anything.**
- **★★ QUANTIZATION RECIPE (`quant_recipe_v1`) — fp16 EVERYTHING, RATIFIED (Matt, 2026-08-15, both repos, incl. pref — overrides the report's pref-fp32 exception).** fp16 halves every head and moves no gate verdict (max ΔAUC 8e-4, zero decoded flips). int8 weight-only FAILS every head without structural work (BN folding / MSE scales / QAT — the named follow-ups): its damage concentrates at the SPARSEST cutpoint (AUC(≥4) moved 1.8× the retrain band). The sensitive group is the final 1280-wide pointwise `conv_head`, NOT first-conv/classifier. Tool `tools/quant/quantize_head.py`; record `data/quant/quant_recipe_v1.json` (tracked); quantized weights staged out-of-tree. Big-repo deploy stays fp32 — the recipe is for SHIPPED weights (fractal-wallpapers applies it after every training).
