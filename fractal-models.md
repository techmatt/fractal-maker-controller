...
`data/ranker/**` has zero commits in history; every emission run ever was UNRANKED and `--intake-floor` a no-op (now raises naming the artifact). The certification record never existed in this checkout — the doc's remembered numbers survive nowhere but `scorer.py`'s old header. **Rebuild BLOCKED on inputs:** the 379 blind labels survive (fractal-corpus) but all three tile→location join manifests were scratch-only and wiped; `campaign1`'s is CIRCULAR (its manifest scored admissions with the ranker before sampling). Path back = re-collect labels and RE-BASE the bar (new batches ⇒ new LOBO folds) — the v11-prep decision. Pre-registered bar + record: `docs/design/deferred_recalibration.md` § Ranker rebuild. Emission ordering degrades to seeded shuffle within partition (fractal-emission). **HARD SCOPE stands: never frontier priority, dive-start, scheduling, or any discovery decision.**
