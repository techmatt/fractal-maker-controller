...
- **★ POST-FLIP AUDIT READ [human n=200, sheet F]:** mining v3's first correction datum — **88.0% exact**, ≥2 flips 5↑/4↓ (symmetric) ⇒ the head's ordering matches Matt; the flip's volume-matched cut was ~7× too tight and is SUPERSEDED by the human-derived crossover (values + lock chain → fractal-thresholds). Under v3 the anchored-vs-blind gap at ≥2 is ~3 pts (numbers → fractal-corpus) — the anchoring prices above are v1-era facts, not properties of correction sheets per se; the ceilings rule stands.
