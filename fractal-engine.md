...
- **★ Smooth's residual cost is ONE parity project, not five items** — the frontier is Python-side f64 memory bandwidth (downsample + LUT), not Rust; target ~500–600 ms from 1192 ms at pool geometry, parity gate on the OUTPUT DECISION. **GPU consequence: an infinitely fast kernel caps the end-to-end win at ~1.3×.**
