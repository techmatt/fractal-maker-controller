...
- **★ Smooth is RUST-BOUND under the ×8 cap [measured 2026-08-03]:** dump median **13,072 ms** (83.4%) vs Python tail 2,232 ms (14.2%) at pool geometry — the cap raise flipped the frontier; the old 1,192 ms tail baseline is unreproducible (basis nowhere in either repo). The tail matters only ≥5.9 recolors/location (emission pool-k=7 sits just past; harvest renders Recipe-1 at zero recolors). **The tail-parity project is RETIRED; the successor attack profiles the Rust kernel (84% of wall, sized not profiled).**
- **★ Recolor-batching executor — build at the next cache rebuild.** `v4-render-batch` re-iterates per plan row, so a palette slot costs the same as a geometry slot; a recolor-batching executor cuts ITERATION work ~4× (not wall). Deliberately kept out of the ckpt-22 re-render; worth more since the cap raise inflated exactly the work it removes.
...
## MACHINE GUARDS
- **Canaries: regenerate the list with `pytest --collect-only` — don't carry it.** Only the non-obvious survive here: `test_q4_screen_parity` (synthetic field + real model, survives corpus wipe) · `field_gating_matches_ungated` (differential) · `test_the_enumeration_is_not_stamped_with_a_cap_policy` (negative-space, pins the exact key set) · `tests/occupancy.rs` (fraction-of-TILES, strict `>` at floor 0.0 vs `guided_descend`'s `<` cull) · ⚠ `tools/v8/test_v8_cache_alignment.py` — fixture SKIPS on absent `data/v8/*.jsonl`, a live absence-tolerant trap.
