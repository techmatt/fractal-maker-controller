...
- **★ Smooth is RUST-BOUND under the ×8 cap [measured]:** dump ~83% of wall vs Python tail ~14% at pool geometry; the tail matters only ≥5.9 recolors/location. **Tail-parity RETIRED; the successor attack profiles the Rust kernel (sized, not profiled).**
- **★★ Recipe 3 `crop-batch` — THE CACHE-BUILD RECIPE FROM v11 ON.** One extended-field iteration pass per LOCATION, every tile crops out of it; fields stream-and-discard (NO field-cache interaction); all draws seeded, `--replay` byte-identical; canonical-frame maxiter per row (a real ~3% policy change vs v9/v10's per-slot rule, accepted). Containment formula, `field_ss 2` pin + grid fact, margin triple, and per-unit costs → **`docs/design/crop_batch.md`** (the two Rust doc comments that contradicted the containment formula were stale, fixed at the extraction — the doc is authoritative).
...
- **★★ TONE MAPPING / EXPOSURE — engine facts → `docs/design/exposure_hdr.md`; keep only the rules:**
  - **★★ NEVER GATE ON CLIP SHARE — use IN-MASK CHROMA.** Every compressive operator drives clip share to 0.000 by construction; the in-engine rolloff can zero the statistic while the lozenge stays visibly white and flat (soft-knee at k=0.35 maps L=1 → **237**/255 — no compressive operator can clip, which is exactly why the statistic lies).
  - **Rolloff reach, precisely:** `apply_rolloff` has exactly two call sites (`MergeMode::Screen`, `Combine::Screen`) — **8 of the 15 roster modes can reach one, exactly 1 has it on (`direct_trap_screen` at `soft_knee@0.35`, working), and the 6 pure-field modes never reach the function.** `direct_trap_lines`/`_ring` ARE reachable and merely render rolloff-off by roster.
  - **`direct_trap_multiply` is UNREACHABLE by any tone operator** — its white is UNHIT BACKGROUND (`start_color: white` + multiply) ⇒ only opacity/threshold or a different start colour can fix it.
  - `transform=histeq` is an EXISTING, PARSED engine flag no committed spec uses (all 15 are `linear`) — the most effective field-side lever measured, zero new code.
