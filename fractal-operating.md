...
## CKPT-56 APPENDIX — new standing rules (integrate into sections at ckpt 57)
- **★★ AUDIT-FIRST (Matt, 2026-08-18):** when a task needs technically-important knowledge the session lacks (engine mechanisms, catalog contents, config truths), STOP and ask Matt whether to send a facts-only audit prompt to the wallpapers repo — never scaffold load-bearing content around CC-fill spots. Audit prompts report facts and never compose prose; the session authors all prose (proven: audit_rendering_modes → v4 placed with zero fill spots).
- **★★ "FULL-SIZED" = 2560×1440 ss4 — final word (Matt, 2026-08-18); ss4 is a RULE everywhere.** A hypothetical 4K future would likely drop to ss2, but not now. Retires every supersample-reduction perf fix permanently.
- **★ Retro passes over placed prose are NORMAL (Matt, 2026-08-18)** — constant revision is the model; no guard ceremony, no listing terms for pre-approval before amending placed sections.
- Website figure provenance is a CHECK-ENFORCED convention (per-panel provenance in the figure registry; full location description, CLI line acceptable for one-offs) — the website repo's CLAUDE.md owns it; prompts need not restate.
