...
- **Delivery (Matt, 2026-08-11):** WHOLESALE replacement docs are emitted as INDIVIDUAL files in outputs and presented — Matt uploads them into the working dir himself; the apply-prompt shrinks to guard + expected file list/sizes + hunks + spot-grep + one commit, and wholesale doc content never travels inside the prompt or through Drive. Exact-hunk distillations keep the in-prompt carrier.
- **Do NOT write the distillation prompt until the era's FINAL report is in** — it always changes it. Corrections to a handed-off distill prompt go as ADDENDUM files, never a rewrite (Matt, 2026-08-11).
...
- **★★ THE STANDING PROMPT CONTRACT LIVES IN fractal-maker's `CLAUDE.md`** (`## Standing prompt contract`, `6634410`) — report shape/path/delivery, runtime discipline, engine-subprocess helpers, commit gate. **Prompts no longer restate any of it**; absorb by cross-reference, never by copy. **The ≤10-line VERDICT block at report top (Matt pastes it into chat) postdates the absorption — state it in prompts until absorbed (candidate absorption into CLAUDE.md).**
- **★ ALL PROMPTS GO THROUGH `prompts\`** — code-work and session/distillation prompts alike; one channel, no exceptions.
- The controller repo's `CLAUDE.md` carries the mirror rules (read prompts from either location, never write under the exchange folder, handoff docs single-sourced there). **CC MAY be pointed at `C:\Code\fractal-maker-controller` directly, even from fractal-maker prompts — READ-ONLY to code-work CC (Matt, 2026-08-11).** `matt-claude-workflow.md` (exchange-folder root) documents the pattern for a NEW project.
