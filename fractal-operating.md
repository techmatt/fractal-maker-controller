...
- **★★ CONTEXT STEWARDSHIP IS PART OF THE SESSION CONTRACT (new 2026-08-11).** Claude tracks its OWN context budget against a **~100k soft ceiling**, and on reaching it **calls "checkpoint now" instead of proposing a further prompt** — the failure mode is proposing work there is no context left to read the result of. Report a token estimate at the checkpoint for cross-session comparison (this session: ~175k).
- **★ THE DRIVE CONNECTOR BASE64-ENCODES EVERY DOWNLOAD regardless of file type — `.txt` is not an escape.** It was the single biggest cost this session. **Decode once, read selectively**; never re-fetch a report to re-read a section.
...
**Confidence convention.** Every verdict carries **basis** (`[human n=X]`·`[machine-decode]`·`[measured]`·`[inferred]`·`[by-eye]`), **population** (most falsified verdicts were population errors wearing a number), and **overturned-by**. A verdict with no falsifier is a belief; `[machine-decode]` is evidence about the MODEL, not the world. **★ A reproducibility test must re-run the writer the artifact came from, not a plausible neighbour** — and a prompt making classification mechanical must PIN the writer (the q4-fields false-durable arc). **★★ A "BYTE-IDENTICAL REBUILD" CLAIM IS PROVEN BY RUNNING THE REAL COMMANDS, FIRST — before anything is de-tracked** (and run with the dependencies already gone, so the proof doesn't rest on them). It is also the step that finds what does NOT reproduce: a live disk probe frozen into a record has no guard and will differ. **★ Early reads of a slow-starting run: mechanism (mix conformance, queue states) is valid immediately; yield/funnel is warm-up, not the run** — this bit twice at ckpt 30. **★★ A STAGE THAT CAN TRUNCATE ITS OWN INPUT ON THE WAY TO FAILING HIDES ITS OWN FAILURE.** Sheet D's freshness glob picked up the sheet's own `images.jsonl` once written, zeroing the population — and the zero-row run still reached `open("w")` and replaced the good artifact with an empty one, which made the NEXT run find nothing to exclude and succeed. Observed as crash/success/crash/success with the tracked file flipping between 197 rows and 0. **Refuse to write when the draw and the ledger share zero rows, BEFORE opening the file** — now standard, and the self-exclusion is fixed separately so the guard is not the only wall.
...
- **★★ THE RETRAIN PROTOCOL IS THE OWNER — POINT, DON'T RESTATE** (`docs/design/classifier_retrain_protocol.md`): **§2a** contradicting stamped splits → global re-derivation (or a frozen authority) · **§2b** anchored correction-sheet labels are train-side only, per-head · **§5a** volume-matching · **§5-0** the pin-marker class. Corpus owns §2a/§2b's measured populations; thresholds owns the cuts.
- **★★ A STAGE-2 FLIP IS NOT A STAGE-1 FLIP — §5's ledger-rescore and τ_h steps are LOCATION-HEAD-SPECIFIC.** `ledger_rescore.py` scores through the location head, so moving a stage-2 pin touches no ledger `p_good` and no τ_h. What DOES carry stage-2 scores — `release_records` + `mining_gate_reports`, 398 + 264 rows — are **RECORDS OF DECISIONS TAKEN, NOT INPUTS**, so they are never rewritten: the `floor` column beside each score says what it was judged against, and the loss is stated as `known_flip_cost` in both adoption records.
- **★ VOLUME-MATCH LANDS THE NEW CUT AT THE MIDPOINT** between the k-th and (k+1)-th score — a report's `cut_at` is the k-th and admits k under `>=` but k−1 under `>`, and **the two stage-2 gate sites disagree** (`emit_v1` uses `>`, `MiningScorer.gate` uses `>=`). Re-count realized volume under the ROUNDED constant, never the swept one.
- **★ A `--limit` RUN STAMPS `incomplete` AND THE LOCK REFUSES IT** (matching the `sitting_cut.INCOMPLETE` rule) — a partial measurement must not be able to become a calibration.
- **★★ A RUNNING TRAINER PINS EVERY MODULE IT IMPORTS.** Windows DataLoader workers are SPAWNED, so each re-imports the module tree fresh: editing a source file mid-run killed a seed with an `AttributeError` on a class pickled from the old definition. **Put everything remaining in ONE chained script and touch no source file until it finishes** — the log-file and detach rules do not cover this one.
- **★★ A RED TEST NOBODY RUNS PROTECTS NOTHING.** `test_flip_end_to_end` sat red in the MANUAL slow lane for two days, calling a function deleted a commit earlier, and was found only because a flip finally went looking. A lane that is not on any standing invocation is not coverage.
