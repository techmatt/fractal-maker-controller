...
**The roster — one doc per subject, grouped by what changes together:** operating (this method) · state (plan/status/flags/manifest — EVERY distillation) · engine (renderer, render canon, families, identity, anchors, machine guards) · storage (repo, durability contract, tracked-bulk policy) · orchestration (production loop, run safety, concurrency) · models (v11, stage-2 heads, screens, aug recipe) · thresholds (floors, τ_h, live cuts) · corpus (labels, sampling, rig, coverage) · discovery (stage 1: sourcing, maneuvers, hunt, harness) · emission (stage 2) · tutorial (changes when: tutorial/transfer work; owns the fractal-wallpapers transfer project) — eleven docs.

**★★ SELECTIVE DISTILLATION — the core rule (Matt).** At a distillation, claude.ai decides which docs the era touched or invalidated and emits ONLY those plus state.md — default a SMALL SUBSET; emitting everything needs a stated reason; an untouched doc cannot grow. **The apply-prompt does the applying:** a `DISTILL_`-prefixed prompt (content guard: working dir must hold the 11 fractal-*.md at root, no Cargo.toml/src/tools — else STOP) carries wholesale replacements + exact hunks, sanity-checks touched files against soft targets, spot-greps single-home, verifies untouched files untouched, one git commit.
...
- **Every prompt opens with a TARGET-REPO guard** naming its repo (`fractal-wallpapers` / `fractal-maker` / `fractal-maker-controller`) with STOP-on-mismatch; the project now spans three repos.
- **★★ CHECKPOINT PROMPT NAMING (Matt, 2026-08-14): `distill_ckptXX.md` is ALWAYS the controller-CC apply prompt (mechanical); `continuation_ckptXX.md` is what a fresh claude.ai session receives alongside the eleven docs.** **A checkpoint produces exactly ONE hand-off artifact (Matt, 2026-08-14; ckpt 44 wrongly shipped both).** If the checkpoint distills (docs change): author `distill_ckptXX.md` alone — the updated doc set, especially state's status/OPEN/Queue, IS the fresh session's hand-in, and any session-specific context a fresh session needs is written INTO state, never into a side file. If a session hands off WITHOUT distilling (context ceiling mid-era, distillation deferred): author `continuation_ckptXX.md` alone, against the unchanged docs. Never both.
...
- **★★ THE DISTILLED-REPO MODEL (Matt, 2026-08-14).** Fresh repo = everything pulled in cleanly, CONTEXT-FREE — no v1→v11 baggage, no side-script ecology. Labels move FLAT: resolved one-row, train/test split PROVIDED as data, the iteration history ≈ a few sentences. NO byte-identity requirement — cleanliness/readability/sanity over byte-identicalness; the bar = a head trained INSIDE the clean repo lands ~within new-seed variance of the big repo's. Shipped pretrained weights are trained WITHIN the small repo, never copied. Editorial line: *"this is about fractals, not ML"* — ML hygiene ≈ a sentence, exposition budget on fractal topics. Per-transfer: Matt+Claude jointly pick the minimal clean form — verbatim copy where sensible, flatten elsewhere.
