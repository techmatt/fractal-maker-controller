...
**`C:\Code\fractal-drive-sync\{prompts,reports,prose,preserve}\`** — Drive-synced, outside all repos: claude.ai writes prompts into `prompts\` and article drafts into `prose\`; CC copies reports into `reports\`. **`preserve\` is DURABLE (Matt, 2026-08-20)** — audits/reference material a future claude.ai or CC needs without doc baggage; Matt never deletes there; use SPARINGLY (sync-drift risk); cite by path, never restate. Everything else is **SCRATCH — Matt deletes freely**; empty/absent is normal. One writer per subfolder; `scratch/` stays a report's canonical home.
...
- **★ ALL PROMPTS GO THROUGH `prompts\`** — code-work and session/distillation prompts alike; one channel, no exceptions. **DELIVERY = a confirmed Drive create into `prompts\` in the same turn the prompt is authored; presenting a file in chat is NOT delivery** (missed twice). Re-resolve the folder id fresh each session; two same-named `prompts` folders exist on Drive (fractal's + seraphinas') — verify by a known fractal prompt inside, never by title.
...
- **★★ AUDIT-FIRST (Matt):** when a task needs technically-important knowledge the session lacks (engine mechanisms, catalog contents, config truths), STOP and ask Matt whether to send a facts-only audit prompt — never scaffold load-bearing content around CC-fill spots. Audits report facts and never compose prose; the session authors all prose (proven twice: audit_rendering_modes, audit_finding_locations).
