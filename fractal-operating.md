...
- **★★ LABELS MUST SERVE OBJECTIVES THE NEXT RETRAIN CANNOT DEPRECATE (Matt, firm, 2026-08-05).** Until a network is believed final-stage, push back on labeling whose value dies with the next version. Passes: calibration coverage via score-unconditioned draws (re-scored under every future head — a one-time purchase), eval instruments, design reads. Fails by default: training volume for the current head.
