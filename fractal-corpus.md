...
**Canonical reader = `label_store.resolve_score` — never write another, never infer labeling state from `images.jsonl`.** Labels live in three registered places (in-store `images.jsonl` + repo-root `labels/*.json` sidecars + the amendment overlay). Labels never-delete, committed; `decoded_class` ≠ human label; location label = max over crops. **8,467 pairs / 7,151 locations at ckpt 25; +730 crawl (81 = rule `interior_gt30_v1`, Matt's >30%-interior guarantee, distinct provenance) +580 harvest since.**
...
- **★★ Four partitions have ZERO eval rows: phoenix, julia:mandelbrot, multibrot3/4/5** — no regression detectable, thresholds uncalibrated. **Phoenix is the one to watch:** 573 training locations and the only partition where class 4 (54) outnumbers class 3 (29), sitting exactly across the new cutpoint; its missing `t_good` is an eval-sampling gap, not a label gap.
- mb3 still has ZERO class-4 anywhere; mb4/mb5 gained 3 each and mandelbrot 17 from the harvest (train-side). Mandelbrot: 0 class-4 in 526 unbiased ⇒ an unbiased draw there is mostly overhead.
...
- **★ The instrument worth building next: fresh unbiased per-family draws, labelled after the fact** — the only genuinely uncontaminated paired instrument, cuttable later so deferring costs nothing. Priority: **phoenix → native multibrot → julia:mandelbrot.** (Rejected: a stratified holdout over the corpus — most of it is model-selected.)
...
- **★ The batch-design template that works:** chunks of ~290, **stratified round-robin across every (source_batch × family) cell to ±1** — a single-family chunk calibrates the bar on that family invisibly. Blindness beyond hiding: opaque ids assigned post-shuffle; a separate blinded manifest served with leak keys DROPPED, not nulled.
